...
import { logVisitor } from "../middleware/logMiddleware";
...
function Alyanslar() {
  ...
  useEffect(() => {
      ...
      logVisitor("/alyanslar", kullaniciAdi);

    const fetchUrunler = async () => {
let query = supabase
  .from("urun")
  .select("*")
  .ilike("kategori", "%alyans%"); // sadece alyans

if (kategoriFilter) {
  query = query.ilike("metal_turu", `%${kategoriFilter}%`);
}
      ...
      if (!error) {
        const filtreliData = data.filter((u) => {
          ...
          return fiyat >= min && fiyat <= max;
        });
        setUrunler(filtreliData);
      }
    };
    fetchUrunler();
  }, [kategoriFilter, minFiyat, maxFiyat]);
  ...
  <div className="filter-group">
    <h4>Metal Türü</h4>
    <button 
      className={kategoriFilter === "altın" ? "active" : ""}
      onClick={() => setKategoriFilter("altın")}
    >
      Altın
    </button>
    <button 
      className={kategoriFilter === "gümüş" ? "active" : ""}
      onClick={() => setKategoriFilter("gümüş")}
    >
      Gümüş
    </button>
    <button 
      className={kategoriFilter === "" ? "active" : ""}
